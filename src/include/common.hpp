...
subsetRandomSeparation( ContainerIn &&items, size_t select, ConverterFn converter )
{
    ...
    {
        auto[it, inserted] = subsetIndices.insert( sampler());
        subsetSize += inserted;
    }
    ...
    auto nextStopIt = subsetIndices.cbegin();
    for (auto i = 0; i < items.size(); ++i)
    {
        if ( nextStopIt != subsetIndices.cend() && i == *nextStopIt )
        {
            ++nextStopIt;
        } else
        {
...
//std::pair<std::vector<std::string> , std::vector<std::pair<char , PairChar >>>
//bytePairEncoding( const std::vector<std::string_view> &sequences , double minimumFrequency )
//{
////    static_assert( std::is_same< uint8_t , char >::value , "Char is assumed here to be unsigned");
//    std::map<PairChar , size_t> pairsFrequency;
//    for ( auto &seq : sequences ) {
//        for ( size_t i = 0; i < seq.length() - 1; ++i ) {
//            ++pairsFrequency[PairChar( seq[i] , seq[i + 1] )];
//        }
//    }
//
//    using PairType = std::pair<PairChar , size_t>;
//    auto cmp = []( const PairType &p1 , const PairType &p2 ) { return p1.second > p2.second; };
//    using ContainerType = std::deque< PairType  >;
//    using PQ = std::priority_queue<PairType , ContainerType , decltype(cmp)>;
//    PQ original( cmp );
//    PQ encoded( cmp );
//
//}
