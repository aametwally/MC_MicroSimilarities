    ...
    public:
                           ...
                           std::optional<std::reference_wrapper<const Selection>> selection = std::nullopt ) const
        {
            switch (classificationStrategy)
            {
                ...
                {
                    MCPropensityClassifier<Grouping> classifier( backbones, background );
                    return classifier.scoredPredictions( queries );
                }
                case ClassificationEnum::DiscriminativePropensity :
                {
                    MCDiscriminativeClassifier<Grouping> classifier( backbones, background );
                    return classifier.scoredPredictions( queries );
                }
                case ClassificationEnum::Segmentation :
                {
                    MCSegmentationClassifier<Grouping> classifier( backbones, background, trainingClusters,
                                                                   _modelTrainer );
                    return classifier.scoredPredictions( queries );
                }
                ...
                {
                    MCKmersClassifier<Grouping> classifier( backbones, background );
                    return classifier.scoredPredictions( queries );
                }
                default:
    ...
    PipelineVariant getConfiguredPipeline( MCModelsEnum model, Order mxOrder, Similarity similarity )
    {
        ...
        switch (model)
        {
            ...
            case MCModelsEnum::GappedMC :
                return Pipeline<AAGrouping>( MG::template create<GMC>( mxOrder ), similarity );
            default:
                throw std::runtime_error( "Undefined Strategy" );
        }
    };
    ...
    getConfiguredPipeline( AminoAcidGroupingEnum grouping, CriteriaEnum criteria, MCModelsEnum model, Order mxOrder )
    {
        switch (grouping)
        {
            case AminoAcidGroupingEnum::NoGrouping20:
                return getConfiguredPipeline<AAGrouping_NOGROUPING20>( criteria, model, mxOrder );
