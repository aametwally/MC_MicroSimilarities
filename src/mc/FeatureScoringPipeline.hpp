...
private:
    ...
    using SimilarityFunction = std::function<double(
            const Histogram &,
            const Histogram &
    )>;
    ...
    struct PredictionFeatureCorrelator
    {
        ...
        std::optional<std::string> assert_feature(
                Order order,
                HistogramID id
        )
        {
        ...
        void addFeatureScoring(
                std::string &&label,
                HeteroHistogramsFeatures &&scoring
        )
        {
...
public:
    ...
    FeatureScoringPipeline(
            ModelTrainer &&modelTrainer,
            HistogramsTrainer &&histogramsTrainer
    )
        ...
        static std::map<std::string, SimilarityFunction> m{
                ...
                {ChiSquared::label,
                        [](
                                const Histogram &k1,
                                const Histogram &k2
                        ) { return ChiSquared::measure( k1, k2 ); }},
                {Intersection::label,
                        [](
                                const Histogram &k1,
                                const Histogram &k2
                        ) { return Intersection::measure( k1, k2 ); }},
                {Gaussian::label,
                        [](
                                const Histogram &k1,
                                const Histogram &k2
                        ) { return Gaussian::measure( k1, k2 ); }},
                ...
                {ItakuraSaitu::label,
                        [](
                                const Histogram &k1,
                                const Histogram &k2
                        ) { return ItakuraSaitu::measure( k1, k2 ); }},
                {"dummy",
                        [&](
                                const Histogram &k1,
                                const Histogram &k2
    ...
    void classify_VALIDATION_SAMPLER(
...
FeatureScoringPipelineVariant getFeatureScoringPipeline(
...
FeatureScoringPipelineVariant getFeatureScoringPipeline(
        const std::string &groupingLabel,
        const std::string &model,
        Order mxOrder
)
{
